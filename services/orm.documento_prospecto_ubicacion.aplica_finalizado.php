...
$documentos = $documento_prospecto_ubicacion_modelo->docs_sin_finalizar(2);
if(errores::$error){
...
foreach ($documentos as $documento){
    ...
    $dbx = $documento_prospecto_ubicacion_modelo->aplica_finalizado($documento['documento_prospecto_ubicacion_id']);
    if (errores::$error) {
        $link->rollBack();
        $error = (new errores())->error('Error al finalizar', $dbx);
        $link->beginTransaction();
        print_r($error);
        exit;
    }
