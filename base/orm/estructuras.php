...
class estructuras{
    ...
    private function asigna_dato_estructura(array $campo, string $name_modelo): array|stdClass
    {
        ...
        $campo_init = $this->inicializa_campo(campo: $campo);
        if(errores::$error){
    ...
    public function asigna_datos_estructura(): array|stdClass
    {
        ...
        $estructura_bd = $this->genera_estructura(modelos:$modelos);
        if(errores::$error){
            return $this->error->error(mensaje: 'Error al maquetar estructura', data: $estructura_bd);
        }
    ...
    private function asigna_datos_modelo(array $data_table, string $name_modelo): array|stdClass
    {
        $estructura_bd = array();
        foreach ($data_table as $campo){

            $estructura_bd = $this->asigna_dato_estructura(campo: $campo,name_modelo: $name_modelo);
            if(errores::$error){
                return $this->error->error(mensaje: 'Error al maquetar estructura', data: $estructura_bd);
            }
    ...
    private function inicializa_campo(array $campo): array|stdClass
    {
        ...
        $es_foranea = $this->es_foranea(campo: $campo);
        if(errores::$error){
            return $this->error->error(mensaje: 'Error al verificar $es_foranea', data: $es_foranea);
        }
        $tabla_foranea = $this->tabla_foranea(campo: $campo);
        if(errores::$error){
    ...
    private function es_foranea(array $campo): bool
    {
        $es_foranea = false;
    ...
    private function genera_estructura(array $modelos): array|stdClass
    {
        $estructura_bd = array();
        ...
        foreach ($modelos as $name_modelo){
            ...
            $estructura_bd = $this->asigna_datos_modelo(data_table: $data_table,name_modelo: $name_modelo);
            if(errores::$error){
                return $this->error->error(mensaje: 'Error al maquetar estructura', data: $estructura_bd);
            }
        }
    ...
    private function tabla_foranea(array $campo): string
    {
        $tabla_foranea = '';
        $explode_campo = explode('_id', $campo['Field']);
        if((count($explode_campo) > 1) && $explode_campo[1] === '') {
